...
const getServerUrl = () => {
  ...
  // For CS environments with specific URL patterns
  if (window.location.hostname.includes('.cs-') || 
      window.location.hostname.includes('.cloudshell.dev')) {
    // Get the base hostname and replace the port
    const currentUrl = window.location.hostname;
    const baseUrl = currentUrl.split('-')[0]; // Get prefix before first dash
    
    // Replace port 3000 with 5000 in the URL
    if (currentUrl.includes('3000-')) {
      const serverUrl = currentUrl.replace('3000-', '5000-');
