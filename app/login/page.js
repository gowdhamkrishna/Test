...
export default function LoginPage() {
    ...
    useEffect(() => {
      ...
      const onUserAdded = (userData) => {
        try {
          // Save the complete user data from the server
      ...
      socket.on('connectionError', onServerError);
  
      // Clean up listeners when component unmounts
      return () => {
        socket.off('UserExist', onUserExist); 
        socket.off('userAdded', onUserAdded);
        socket.off('connectionError', onServerError);
      };
    }, [formData, router]);
    ...
    const handleSubmit = (e) => {
      ...
      const connectionErrorHandler = (error) => {
        ...
        toast.error(error.message || "Unable to connect to the server. Please try again later.");
        setLoader(false);
      };
